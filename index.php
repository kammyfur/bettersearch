...
<body>
    <div vcenter>
        <center>
            <p><img src="/plugins/fulllogo_dark.png" alt="<?= $lang->imgalt ?>" logo></p>
            <p><?= $lang->trigger ?></p>
            <?php
            ...
            ?>
            <p><input type="text" class="disabling" placeholder="<?= $lang->ph ?> <?= $lang->name ?>" searchbar></p>
            <p><input onclick="submit();" class="disabling" type="button" value="<?= $lang->search ?>" searchbutton button> &nbsp; &nbsp; &nbsp; <input onclick="location.href = '/init'" class="disabling" type="button" value="<?= $lang->settings ?>" settingsbutton button></p>
            <p mobilenav>
                <small>
    ...
    SuggestionsAPI = horsey(document.querySelector("[searchbar='']"), {
        source (data, done) {
            $.ajax({
                type: "GET",
                dataType: 'html',
                url: "/api/suggestions/?q=" + document.querySelector('[searchbar=""]').value + "&mode=search&limit=5",
